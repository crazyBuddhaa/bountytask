...
function buildVerificationCriteria(task: Task): string {
...
${buildVerificationCriteria(task)}
...
- "rejected" — screenshot does not prove the action, shows wrong account/post, appears edited, or is unrelated (use confidence 0–35)`
}

/**
 * Verifies a social media task completion screenshot using Gemini Vision.
 * Returns a verdict, confidence score, and reason.
 * On any error (network, API, parse failure) returns "uncertain" so the
 * completion falls through to manual review rather than blocking the user.
 */
export async function verifySocialScreenshot(
  ...
  if (!process.env.GEMINI_API_KEY) {
    console.error("GEMINI_API_KEY is not set — social task AI verify falling back to manual review")
    return {
      verdict: "uncertain",
      confidence: 0,
  ...
  try {
    // Fetch the image from Cloudinary and convert to base64 inline data
    ...
    const result = await model.generateContent({
      ...
      generationConfig: {
        ...
        maxOutputTokens: 256,
        temperature: 0.1, // low temperature = consistent, literal decisions
      },
    })
    ...
    // Normalise: enforce confidence → verdict mapping regardless of what the model said
    ...
    // Any failure (network, quota, parse error) → uncertain so the user isn't
    // silently blocked. Admin will review normally.
