...
import { getAsterraSettings, buildAsterraSmartlinkUrl } from "@/lib/asterra"
...
export default async function SmartlinkPage() {
  ...
  const wallUrl   = settings.smartlinkUrl
    ? buildAsterraSmartlinkUrl(settings.smartlinkUrl, user.id)
    : ""
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ExternalLink className="w-6 h-6" /> Asterra Smartlink
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Complete offers, app installs, surveys, and sign-ups — Asterra auto-selects the best
            available offer for your profile.
          </p>
        </div>
      ...
      {!settings.enabled || !wallUrl ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Smartlink offers aren&apos;t available yet. Check back later.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : capHit ? (
        <Card>
          <CardContent className="p-8 text-center space-y-3">
            <ExternalLink className="w-10 h-10 mx-auto text-muted-foreground/40" />
            <p className="font-medium">All done for today!</p>
            <p className="text-sm text-muted-foreground">
              You&apos;ve hit the daily limit of {settings.dailyCap} completion{settings.dailyCap === 1 ? "" : "s"}.
              Come back tomorrow.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="overflow-hidden p-0">
          <iframe
            ...
            title="Asterra Smartlink Offers"
      ...
      <Card className="bg-muted/40">
        <CardContent className="p-4 text-xs text-muted-foreground space-y-1">
          ...
          <ul className="list-disc list-inside space-y-0.5">
            ...
            <li>Rewards are credited automatically once the advertiser confirms your conversion.</li>
