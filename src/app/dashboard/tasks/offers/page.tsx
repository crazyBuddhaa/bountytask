...
import { getAyetSettings, buildAyetWallUrl } from "@/lib/ayet"
...
export default async function OffersPage() {
  ...
  const [settings, usedToday] = await Promise.all([
    getAyetSettings(),
    getAdCompletionsTodayCount(user.id, "ayet"),
  ])
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Layers className="w-6 h-6" /> Surveys &amp; Offers
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Complete surveys, sign-ups, and offers from verified advertisers.
          </p>
        </div>
      ...
      {/* Daily cap progress */}
      {settings.enabled && settings.placementKey && (
        <Card>
